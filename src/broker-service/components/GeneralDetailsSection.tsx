...
export const GeneralDetailsSection: FC<GeneralDetailsSectionProps> = ({ namespace, nameError }) => {
    ...
    <FormSection title={t('General Details')} style={{ marginTop: 0 }}>
      <FormGroup label={t('Name')} isRequired fieldId="broker-service-name">
        <TextInput
          ...
          value={name}
          onChange={(_event, value) => dispatch({ type: 'SET_NAME', payload: value })}
          validated={nameError ? 'error' : 'default'}
          placeholder="my-broker-service"
            ...
            <StackItem key={index}>
              <Split hasGutter>
                ...
                <SplitItem style={{ display: 'flex', alignItems: 'center' }}>
                  <Button
                    variant="plain"
                    onClick={() => dispatch({ type: 'REMOVE_LABEL', payload: index })}
                    aria-label={t('Remove label')}
                    icon={<TimesIcon color="red" />}
                  />
                </SplitItem>
              </Split>
            </StackItem>
