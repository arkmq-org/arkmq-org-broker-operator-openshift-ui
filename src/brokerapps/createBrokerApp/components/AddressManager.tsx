...
export const AddressManager: React.FC = () => {
  ...
  // resets address card to default if only one remains instead of removing
  const handleRemove = (index: number) => {
    if (state.addresses.length === 1) {
      dispatch({
        type: 'UPDATE_ADDRESS',
        payload: { index, address: '', ownership: 'private', direction: 'produces', pubSub: false },
      });
      setTouched(new Set());
      return;
    }
  ...
  const isBlankAddress = (entry: { address: string; ownership: string; direction: string }) =>
    !entry.address && entry.ownership === 'private' && entry.direction === 'produces';
    ...
    <FormSection title={t('Addresses')}>
      ...
      <div className={`${PREFIX}-grid`} data-test="address-list">
        {state.addresses.map((entry, index) => {
          const errorMessage = touched.has(index) ? errors[index] : undefined;
                  ...
                  actions: (
                    <>
                      ...
                      <Button
                        ...
                        isDisabled={state.addresses.length === 1 && isBlankAddress(entry)}
                        onClick={() => {
                          handleRemove(index);
                        }}
                        icon={<TimesIcon />}
...
const AddressEditModal: React.FC<{
  ...
  const directionError =
    !localProduces && !localConsumes && localOwnership !== 'shared'
      ? 'At least one direction is required for non-shared addresses'
      : undefined;
  const subscriptionError =
    !isExternal && localPubSub && localConsumes && !localSubscriptions.length
      ...
      : undefined;
  const isModalValid = !requiredError && !directionError && !subscriptionError;
  ...
  return (
    <Modal variant="medium" isOpen onClose={onCancel} aria-labelledby="edit-address-title">
      ...
      <ModalBody>
        <Stack hasGutter>
          <StackItem>
            <FormGroup label={t('Address')} isRequired fieldId={`address-name-${String(index)}`}>
              <TextInput
          ...
          <StackItem>
            <Split hasGutter>
              <SplitItem isFilled>
                <FormGroup label={t('Ownership')} fieldId={`address-ownership-${String(index)}`}>
                  <Select
                    isOpen={openOwnership}
                    onOpenChange={(isOpen) => {
                      setOpenOwnership(isOpen);
                    }}
                    onSelect={(_e, val) => {
                      setLocalOwnership(val as AddressOwnership);
                      setOpenOwnership(false);
                    }}
                    selected={localOwnership}
                    toggle={(toggleRef) => (
                      <MenuToggle
                        ref={toggleRef}
                        isFullWidth
                        ...
                        isExpanded={openOwnership}
                        data-test={`address-ownership-${String(index)}`}
                      >
                        {t(OWNERSHIP_OPTIONS.find((o) => o.value === localOwnership)?.label ?? '')}
                      </MenuToggle>
                    )}
                  >
                    <SelectList>
              ...
              <SplitItem isFilled>
                <FormGroup label={t('Direction')} role="group">
                  <Checkbox
                  ...
                  {directionError && (
                    <FormHelperText>
                      <HelperText>
                        <HelperTextItem variant="error">{t(directionError)}</HelperTextItem>
                      </HelperText>
                    </FormHelperText>
                  )}
                </FormGroup>
