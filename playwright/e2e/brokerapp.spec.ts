...
`;
  ...
  test.describe('BrokerApp list page', () => {
    ...
    test('search by name filters the list', async ({ page }) => {
      await page.locator('[data-test="brokerapp-search"]').fill('matching');
    ...
    test('clicking the provisioned service link navigates to service detail', async ({ page }) => {
      // e2e-app-matching has provisionedService set by the lifecycle test above,
      // so the ResourceLink renders in its list row.
      const serviceLink = page.locator(`[data-test="resource-link-${SERVICE_NAME}"]`).first();
